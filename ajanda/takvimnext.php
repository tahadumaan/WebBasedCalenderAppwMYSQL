...
$strIlkGun = date("Y-m-01", strtotime($strTarihnext));
$strSonGun = date("Y-m-t", strtotime($strTarihnext));
...
$strAyIsim = date("F", strtotime($strTarihnext));
?>
...
<table border="1" cellspacing="0" cellpadding="5" width="90%" height="60%" style="background-color: #EAEAEA">
    ...
    <?php while ($strTakvimGun <= $strTakvimSon) { ?>
        ...
        <?php if (date("m", strtotime($strTakvimGun)) == date("m", strtotime($strTarihnext))) { ?>
            <td style="width:55px; text-align:center" <?php if(((date("N", strtotime($strTakvimGun))) == 6) or ((date("N", strtotime($strTakvimGun))) == 7) ){ ?> bgcolor="a7e0e0" <?php } ?>
                <?php $strSQL="SELECT * FROM ajanda WHERE tablo_tarih='$strTakvimGun'";
                $res= $db->query($strSQL);
                $cnt=$res->num_rows;
                if($cnt>0){
                    ?> bgcolor="#f7ffaa" <?php
